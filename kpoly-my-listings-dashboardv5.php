...
if (!class_exists('KPoly_My_Listings_Dashboard')) {
    ...
    class KPoly_My_Listings_Dashboard {
        ...
        public function __construct() {
			...
			add_action('init', [$this, 'add_my_listings_endpoint']);
			add_filter('woocommerce_account_menu_items', [$this, 'customize_account_menu'], 40);
			add_action('woocommerce_account_my-listings_endpoint', [$this, 'render_my_listings_endpoint']);
		}
		public function add_my_listings_endpoint() {
		...
		foreach ($items as $key => $label) {
			if (in_array($key, ['orders', 'downloads', 'edit-address'], true)) {
				continue;
			}

			if ($key === 'dashboard') {
				$new_items[$key] = 'My Account';
				$new_items['my-listings'] = 'My Listings';
				continue;
			}
		...
		public function render_my_listings_endpoint() {
			echo do_shortcode('[kpoly_my_listings]');
		}
				...
				<div class="kpoly-edit-card">
					...
					<form method="post">
						...
						<div class="kpoly-edit-actions">
							...
							<a href="<?php echo esc_url(wc_get_account_endpoint_url('my-listings')); ?>" class="kpoly-edit-btn kpoly-edit-btn-secondary">Back to My Listings</a>
						</div>
					</form>
				</div>
			</div>
